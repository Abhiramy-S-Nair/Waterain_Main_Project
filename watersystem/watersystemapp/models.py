...
class DeliveryBoyAssignment(models.Model):
    order = models.ForeignKey(ProductOrder, on_delete=models.CASCADE)
    delivery_boy = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
...
class OrderPayment(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    cart = models.ForeignKey(Cart, on_delete=models.CASCADE, default=1)  # Default cart ID, change if needed
    total_amount = models.DecimalField(max_digits=10, decimal_places=2)
    delivery_address = models.TextField()
    payment_id = models.CharField(max_length=100)  # Store Razorpay payment ID or any other payment gateway ID
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"OrderPayment #{self.id} by {self.user.user.username}"